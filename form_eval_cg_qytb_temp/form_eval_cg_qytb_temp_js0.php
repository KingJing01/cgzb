...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_eval_cg_qytb_temp"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function sc_btn_sc_btn_0()
{
    ...
    if (confirm("<?php echo html_entity_decode('已保存选中?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>"))
    {
      nm_gp_submit('<?php echo $this->Ini->link_form_eval_plan_batchset_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "cpbbh?#?" . str_replace("'", "@aspass@", $_SESSION['v_cpbmc']) . "?@?SC_glo_par_cpbbh?#?v_cpbmc?@?"; ?>', '', '_self', '0', '0', 'form_eval_plan_batchset');
    }
    else
    {
      return;
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_eval_cg_qytb_temp']['where_detal']))
  {
?>  
...
{ 
    ...
    Nm_Proc_Atualiz = true;
    do_ajax_form_eval_cg_qytb_temp_submit_form(y, z); 
} 
<?php
}
?>
