...
class grid_notice_xml
{
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_notice']['xml_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_notice']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_notice']['xml_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_notice']['xml_name']);
      }
      if (!$this->Grava_view)
      {
...
$_SESSION['scriptcase']['grid_notice']['contr_erro'] = 'off'; 
   ...
   function NM_export_title()
   {
         ...
         $this->xml_registro .= " title =\"" . $this->trata_dados($this->title) . "\"";
   }
   //----- author
   function NM_export_author()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->author))
         {
             $this->author = sc_convert_encoding($this->author, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " author =\"" . $this->trata_dados($this->author) . "\"";
   }
   //----- releasetime
   function NM_export_releasetime()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->releasetime))
         {
             $this->releasetime = sc_convert_encoding($this->releasetime, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " releasetime =\"" . $this->trata_dados($this->releasetime) . "\"";
   }
   //----- id
   ...
   function monta_html()
   {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_notice']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_notice']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - <?php echo $this->Ini->Nm_lang['lang_tbl_notice'] ?> :: XML</TITLE>
