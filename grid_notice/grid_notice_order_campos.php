<?php
   ...
   if (!function_exists("NM_is_utf8"))
   {
    ...
    $Ord_Cmp = new grid_notice_Ord_cmp(); 
    $Ord_Cmp->Ord_cmp_init();
   
class grid_notice_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   $tab_def_campos['title'] = "title";
   $tab_converte["title"]   = "title";
   $tab_labels["title"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_notice']['labels']["title"])) ? $_SESSION['sc_session'][$sc_init]['grid_notice']['labels']["title"] : "" . $this->Nm_lang['lang_notice_fld_title'] . "";
   $tab_ger_campos['author'] = "on";
   $tab_def_campos['author'] = "author";
   $tab_converte["author"]   = "author";
   $tab_labels["author"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_notice']['labels']["author"])) ? $_SESSION['sc_session'][$sc_init]['grid_notice']['labels']["author"] : "" . $this->Nm_lang['lang_notice_fld_author'] . "";
   $tab_ger_campos['releasetime'] = "on";
   $tab_def_campos['releasetime'] = "releasetime";
   $tab_converte["releasetime"]   = "releasetime";
   $tab_labels["releasetime"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_notice']['labels']["releasetime"])) ? $_SESSION['sc_session'][$sc_init]['grid_notice']['labels']["releasetime"] : "" . $this->Nm_lang['lang_notice_fld_releasetime'] . "";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_notice']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_notice']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_notice']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_notice']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_notice']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_notice']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_notice']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_notice']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       {
           $_SESSION['sc_session'][$sc_init]['grid_notice']['ordem_select'][$campo] = $ordem;
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   $.ajax({
    type: "POST",
    url: "grid_notice_order_campos.php",
    data: {
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (!isset($_SESSION['sc_session'][$sc_init]['grid_notice']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['grid_notice']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
