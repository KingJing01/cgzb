...
var nm_num_campo = 1;
function lookup_app_name_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.app_name_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('sec_form_sec_groups_apps')?>/sec_form_sec_groups_apps.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&script_case_session=<?php echo session_id() ?>&nmgp_opcao=lookup", recebe_app_name_, "app_name_", dados, false);
}
function recebe_app_name_(string)
{
   document.getElementById('id_lookup_app_name_' + nm_num_campo).innerHTML = string;
}
<?php 
