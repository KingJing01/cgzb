...
$_SESSION['scriptcase']['sec_form_sec_groups_apps']['contr_erro'] = 'off';
      ...
      if (!$run_iframe && isset($_SESSION['scriptcase']['menu_atual']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_sec_groups_apps']['embutida_call'] && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_sec_groups_apps']['sc_outra_jan']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_sec_groups_apps']['sc_outra_jan']))
      {
          ...
          if ($this->Ini->sc_page == $this->sc_init_menu && !isset($_SESSION['scriptcase']['menu_apls'][$_SESSION['scriptcase']['menu_atual']][$this->sc_init_menu]['sec_form_sec_groups_apps']))
          {
               ...
               $_SESSION['scriptcase']['menu_apls'][$_SESSION['scriptcase']['menu_atual']][$this->sc_init_menu]['sec_form_sec_groups_apps']['label'] = "" . $this->Ini->Nm_lang['lang_othr_frmu_titl'] . " - " . $this->Ini->Nm_lang['lang_tbl_sec_groups_apps'] . "";
      ...
      $_SESSION['scriptcase']['error_icon']['sec_form_sec_groups_apps']  = "<img src=\"" . $this->Ini->path_icones . "/scriptcase__NM__icnMensagemAlerta.png\" style=\"border-width: 0px\" align=\"top\">&nbsp;";
...
$_SESSION['scriptcase']['sec_form_sec_groups_apps']['contr_erro'] = 'off'; 
      ...
      { 
          if ($GLOBALS['F'] == "app_name_")
          { 
              $nm_parms   = substr($GLOBALS['P0'], 1, strlen($GLOBALS['P0']) - 2);
              $array_vars = explode(",", $nm_parms);
              $this->app_name_ = $array_vars[0];
              $app_name_       = $this->app_name_;
              $this->app_name_       = $app_name_;
              $this->lookup_app_name_($conteudo);
   ...
   function controle()
   {
      ...
      if ($this->NM_ajax_flag && 'validate_' == substr($this->NM_ajax_opcao, 0, 9))
      {
         ...
         $Campos_Falta = array();
         $Campos_Erros = array();
          if ('validate_app_name_' == $this->NM_ajax_opcao)
          {
              $this->Valida_campos($Campos_Crit, $Campos_Falta, $Campos_Erros, 'app_name_');
          }
...
//
//--------------------------------------------------------------------------------------
   function lookup_app_name_(&$conteudo)
   {
      global  $app_name_;
      $this->nm_tira_formatacao();
      $this->formatado = false;
      $nm_comando = "SELECT description 
FROM sec_apps 
WHERE app_name = '$this->app_name_' 
...
//---------------------------------------------------------
     ...
     if ('' == $filtro && isset($this->nm_form_submit) && '1' == $this->nm_form_submit && $this->scCsrfGetToken() != $this->csrf_token)
     {
      ...
      if ('' == $filtro || 'app_name_' == $filtro)
        $this->ValidateField_app_name_($Campos_Crit, $Campos_Falta, $Campos_Erros);
    ...
    function ValidateField_app_name_(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
        global $teste_validade;
      if ($this->nmgp_opcao != "excluir" && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_sec_groups_apps']['php_cmp_required']['app_name_']) || $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_sec_groups_apps']['php_cmp_required']['app_name_'] == "on")) 
      { 
          if ($this->app_name_ == "")  
          { 
              $Campos_Falta[] =  "" . $this->Ini->Nm_lang['lang_sec_app_name'] . "" ; 
              if (!isset($Campos_Erros['app_name_']))
              {
                  $Campos_Erros['app_name_'] = array();
              }
              $Campos_Erros['app_name_'][] = $this->Ini->Nm_lang['lang_errm_ajax_rqrd'];
                  if (!isset($this->NM_ajax_info['errList']['app_name_']) || !is_array($this->NM_ajax_info['errList']['app_name_']))
                  {
                      $this->NM_ajax_info['errList']['app_name_'] = array();
                  }
                  $this->NM_ajax_info['errList']['app_name_'][] = $this->Ini->Nm_lang['lang_errm_ajax_rqrd'];
          } 
      } 
      if ($this->nmgp_opcao == "incluir")
      { 
          if (NM_utf8_strlen($this->app_name_) > 128) 
          { 
              ...
              $Campos_Erros['app_name_'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 128 " . $this->Ini->Nm_lang['lang_errm_nchr'];
              if (!isset($this->NM_ajax_info['errList']['app_name_']) || !is_array($this->NM_ajax_info['errList']['app_name_']))
              {
                  $this->NM_ajax_info['errList']['app_name_'] = array();
              }
              $this->NM_ajax_info['errList']['app_name_'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 128 " . $this->Ini->Nm_lang['lang_errm_nchr'];
          } 
      } 
    } // ValidateField_app_name_
...
//
   ...
   function ajax_return_values_all_vert()
   {
          ...
          $aLookupOrig = $aLookup;
              ...
              $this->app_name_ = $aRecData['app_name_'];
              $orig_app_name_ = $this->app_name_;
              $app_name_      = $this->app_name_;
              $this->app_name_ = $app_name_;
              $this->lookup_app_name_($conteudo);
              $this->app_name_ = $orig_app_name_;
              $this->NM_ajax_info['fldList']['app_name_' . $sc_seq_vert]['lookupCons'] = sec_form_sec_groups_apps_pack_protect_string($conteudo);
              }
...
if (isset($this->NM_ajax_flag) && $this->NM_ajax_flag)
{
    ...
    {
        $this->NM_ajax_info['fldList']['app_name_' . $this->nmgp_refresh_row]['type']    = 'text';
      ...
      { 
          ...
          { 
              ...
              if (isset($this->Embutida_form) && $this->Embutida_form)
              {
                  ...
                  $this->lookup_app_name_($conteudo);
                  $this->app_name_ = $orig_app_name_;
                  $this->NM_ajax_info['fldList']['app_name_' . $this->nmgp_refresh_row]['lookupCons'] = sec_form_sec_groups_apps_pack_protect_string($conteudo);
                  $this->NM_ajax_info['fldList']['app_name_' . $this->nmgp_refresh_row]['type']    = 'text';
...
//---------- 
   ...
   { 
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_sec_groups_apps']['where_filter']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_sec_groups_apps']['where_filter'])
      {
...
function nmgp_redireciona($tipo=0)
{
   ...
   if ($tipo == 2 && isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_sec_groups_apps']['sc_outra_jan']) && $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_sec_groups_apps']['sc_outra_jan'])
   {
